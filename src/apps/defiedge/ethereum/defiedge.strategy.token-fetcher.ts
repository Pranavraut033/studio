...
export class EthereumDefiEdgeStrategyTokenFetcher implements PositionFetcher<AppTokenPosition> {
  constructor(
  ...
  async getPositions() {
    const endpoint = `${baseUrl}/mainnet/strategies`;
    const multicall = this.appToolkit.getMulticall(network);

    const strategies = await axios.get<Strategy[]>(endpoint).then(({ data }) => data);

    const baseTokenAddressToDetails = await this.appToolkit.getBaseTokenPrices(network).then(baseTokens => {
      return _.keyBy(baseTokens, e => e.address.toLowerCase());
    });

    const appTokens = await Promise.all(
      strategies.map(async strategy => {
        const erc20Contract = this.defiEdgeContractFactory.erc20({ address: strategy.address, network });
